...
impl<S: Syscalls, C: Config> Rng<S, C> {
    pub fn fill_buffer(buf: &mut [u8]) -> bool {
        ...
        >(|handle| {
            ...
            // configures the subscription to automatically set `is_filled` to true as soon as the buffer is filled
            let subscription =
            ...
            // request the random number generator to fill the buffer
            ...
            // yield until the buffer is filled
